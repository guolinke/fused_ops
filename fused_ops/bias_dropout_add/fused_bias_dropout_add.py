...
class BiasDropoutAddFunction(torch.autograd.Function):
    ...
    def forward(ctx, input, bias, residual, is_training, dropout_prob):
        ...
        ctx.save_for_backward(mask, torch.tensor([dropout_prob]))
        return result

    @staticmethod
    def backward(ctx, grad_output):
        mask, dropout_prob = ctx.saved_tensors
        if mask is None:
            grad_input = grad_output
        else:
            grad_input = fused_bias_dropout_add_cuda.backward(grad_output, mask, dropout_prob[0])
